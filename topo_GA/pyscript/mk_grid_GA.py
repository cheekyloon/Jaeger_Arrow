...
1.  Loads a high-resolution (HR) bathymetry around the Grande-Anse (GA) wharf (in MTM Zone 7).
2.  Loads a lower-resolution (LR) bathymetry around the GA wharf. 
...
5.  Filled missing values near the coast with zero-gradient extrapolation in the LR bathymetry.
6.  Applies a digitized shoreline mask to exclude land areas from the domain.
7.  Examines the differences between the high- and low-resolution bathymetries 
    within the overlap zone, visualizes these differences, and computes an average 
    vertical offset to correct systematic biases.
8.  Applies the calculated offset to align the low-resolution bathymetry with the high-resolution survey.
9.  Merges the datasets, preserving the high-resolution survey where available.
10. Produces a masked bathymetry field ready for use in MITgcm (.bin format).

Input files:
- High-resolution (0.3 m) .xyz file (x, y in meters, z in meters relative to NMMB)
- Wharf coordinates (.dat file with 2 reference points)
- Low-resolution (10 m) .txt file from CHS NONNA bathymetry
- Digitized shoreline mask (.dat file created manually using DigitTable)
- Coastline .mat file (DB)

Vertical references:
- The (HR) bathymetry is referenced to mean low water level (Niveau moyen à marée basse – NMMB),
  which is 2.76 meters below the Canadian Geodetic Vertical Datum 1928 (CGVD28) at Grande-Anse.
- The CHS NONNA bathymetry (https://ouvert.canada.ca/data/fr/dataset/d3881c4c-650d-4070-bf9b-1e00aabf0a1d)
  is referenced to chart datum (CD), a locally derived tidal datum
...
# Apply correction to HR bathymetry: adds a negative value (effectively subtracts 0.14 m)
df['z'] = df['z'] + dz
...
z_ga    = interpolate_to_grid(df[['x_rot', 'y_rot']].values, df['z'].values, grid_x, grid_y) 
z_fjord = interpolate_to_grid(coords_fjord_rot, Z_sub, grid_x, grid_y)

# Filled missing values near the coast in the LR bathymetry 
z_fjord_filled = fill_holes_nearest(z_fjord)
...
z_ga           = np.where(mask_polygon, z_ga, np.nan)
z_fjord_filled = np.where(mask_polygon, z_fjord_filled, np.nan)

### === Visualize the difference between bathymetries and compute offset === ###
# Visualize the difference between HR and LR bathymetries within the HR zone
diff = plot_bathy_difference(grid_x, grid_y, z_ga, z_fjord_filled, show_figure=True)
...
z_fjord_aligned = z_fjord + offset

### === Merge bathymetries, prioritizing HR data where available === ### 
z_combined = np.where(~np.isnan(z_ga), z_ga, z_fjord_filled)
