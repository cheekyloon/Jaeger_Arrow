...
dz = rsk.compute_dz(all_z, pressure_data)

# Step 4: Compute the weighted average for the temperature along the depth
T_weight = rsk.weight_avg_z(all_T, dz=dz)
...
N_but = 4
# apply filter
T_but     = rsk.bp_filter(T_weight, df_pressure.index, Fc_low, Fc_high, N_but)
...
T_but_weight = rsk.weight_avg_z(np.abs(all_T_but), dz=dz)
...
phase_bin, T_weight_bin = rsk.M2_phase_avg(df_pressure, T_but_weight, N_bin)
...
T = T_weight - np.nanmean(T_weight)
...
ax1[1].plot(phase_bin, T_weight_bin, linestyle='-', color='b', label='Method 2')
ax1[1].legend()
ax1[1].set_ylabel('Depth-Weighted Temperature')
ax1[1].set_xlabel('Tidal Phase')
ax1[1].set_title(f"Bandpass Filtered Periods: {int(T_low)}s – {int(T_high)}s")

# Save figure
figname1 =f"{figdir}phase-avg-BP{int(T_low)}–{int(T_high)}-y{int(year)}.png" 
...
ax3[1].set_ylabel('Depth-Weighted Averaged Temperature')
ax3[1].set_title(f"Method 1: Bandpass Filtered Periods: {int(T_low)}s – {int(T_high)}s")

# Save figure
figname3 =f"{figdir}method1-TS-BP{int(T_low)}–{int(T_high)}-y{int(year)}.png" 
...
# Second subplot: weighted-averaged temperature 
ax4[1].plot(df_pressure.index, T_but_weight, linestyle='-', color='r')
...
ax4[1].set_ylabel('Depth-Weighted Averaged Temperature')
ax4[1].set_title(f"Method 2: Bandpass Filtered Periods: {int(T_low)}s – {int(T_high)}s")

# Save figure
figname4 =f"{figdir}method2-TS-BP{int(T_low)}–{int(T_high)}-y{int(year)}.png" 
...
# Save figure
figname5 =f"{figdir}method1-PSD-BP{int(T_low)}–{int(T_high)}-y{int(year)}.png" 
...
#bin_indices = np.digitize(iphi, bin_edges) - 1 
